...
namespace NStar.BigCollections
{
	...
	public class BigBitList : BigList<bool, BigBitList, BitList>
	{
			...
			: this(subbranchesBitLength, leafSizeBitLength)
		{
			...
			using CustomBigList<uint> values = [];
			var n = 0;
			while (b)
			{
				var i = 0;
			...
			: this(subbranchesBitLength, leafSizeBitLength)
		{
			...
			else if (uints is CustomBigList<uint> bigUIntList)
			...
			{
				{
					using CustomBigList<uint> list = new(uints);
					ConstructFromUIntList(list);
				}
			}
#if VERIFY
		...
		{
			using CustomBigList<uint> list = new(values);
			ConstructFromUIntList(list);
#if VERIFY
			if (high is not null)
				Debug.Assert((highLength is null || Length == highLength?.ValuesSum) && Length == high.Sum(x => x.Length));
		...
		public override BigBitList Add(bool item)
		{
		...
		protected virtual void ConstructFromUIntList(CustomBigList<uint> bigUIntList, int overrideLength = 0)
		{
		...
		protected virtual void ConstructFromUIntListFromScratch(CustomBigList<uint> bigUIntList, int overrideLength = 0)
		{
			Debug.Assert((low is null || low.Capacity == 0) && high is null
				&& fragment == 1 && _capacity == 0);
			ArgumentOutOfRangeException.ThrowIfNegative(overrideLength);
		...
		public virtual CustomBigList<uint> ToUIntBigList()
		{
			if (low is not null)
...
namespace NStar.BigCollections.Common
{
	/// <summary>
	/// Представляет строго типизированный список элементов, упорядоченных по индексу.
	/// В отличие от <see cref="List{T}"/> и стандартного <see cref="G.List{T}"/>, имеет индекс типа <see cref="MpzT"/>, а не
...
namespace NStar.BigCollections.FastInsertion
{
	/// <summary>
	/// Представляет строго типизированный список элементов, упорядоченных по индексу.
	/// В отличие от <see cref="List{T}"/> и стандартного <see cref="G.List{T}"/>, имеет индекс типа <see cref="MpzT"/>, а не
	...
	public class BigList<T> : BigList<T, BigList<T>, LimitedBuffer<T>>
	{
		public BigList() : base(5, 10) { }

		public BigList(MpzT capacity)
			: base(capacity, 5, 10) { }

		public BigList(G.IEnumerable<T> collection)
			: base(collection, 5, 10) { }

		public BigList(MpzT capacity, G.IEnumerable<T> collection)
			: base(capacity, collection, 5, 10) { }

		public BigList(T[] values)
			: base(values.AsEnumerable(), 5, 10) { }

		public BigList(ReadOnlySpan<T> values)
			: base(values, 5, 10) { }

		public BigList(MpzT capacity, T[] values)
			: base(capacity, values.AsEnumerable(), 5, 10) { }

		public BigList(MpzT capacity, ReadOnlySpan<T> values)
			: base(capacity, values, 5, 10) { }
