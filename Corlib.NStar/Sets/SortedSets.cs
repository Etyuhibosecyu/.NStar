...
public class SumSet<T> : BaseSortedSet<(T Key, int Value), SumSet<T>>
{
	...
	private void AddAllElements(IEnumerable<(T Key, int Value)> collection)
	{
		foreach (var item in collection)
		{
			if (!Contains(item))
	...
	/// an ElementCount struct; however this was changed due to the perf overhead of delegates.
	/// </summary>
	private unsafe ElementCount CheckUniqueAndUnfoundElements(IEnumerable<(T Key, int Value)> other, bool returnIfUnfound)
	{
		ElementCount result;
		// need special case in case this has no elements.
		if (Length == 0)
	...
	public virtual bool Contains(T? item) => item != null && TryGetValue(item, out int _);

	private bool ContainsAllElements(IEnumerable<(T Key, int Value)> collection)
	{
		foreach (var item in collection)
		{
			if (!Contains(item))
	...
	private protected override void CopyToInternal(int index, (T Key, int Value)[] array, int arrayIndex, int count)
	{
		ArgumentNullException.ThrowIfNull(array);
		if (index < 0)
			throw new ArgumentOutOfRangeException(nameof(index));
	...
	public override void ExceptWith(IEnumerable<(T Key, int Value)> other)
	{
		ArgumentNullException.ThrowIfNull(other);
		if (_size == 0)
			return;
	...
	internal virtual Node? FindNode(T item)
	{
		...
		while (current != null)
		{
			int order = Comparer2.Compare(item, current.Item.Key);
			if (order == 0)
				return current;
			current = order < 0 ? current.Left : current.Right;
		}
	...
	public virtual T GetAndRemove(Index index)
	{
		...
		if (root == null)
			return default!;
		FindForRemove(index2, out SumSet<T>.Node? parent, out SumSet<T>.Node? grandParent, out SumSet<T>.Node? match, out SumSet<T>.Node? parentOfMatch);
		T found = default!;
		// Move successor to the matching node position and replace links.
		if (match != null)
		{
	...
	/// <returns>A value indicating whether both sets have the same comparer.</returns>
	private bool HasEqualComparer(SumSet<T> other) => Comparer == other.Comparer || Comparer.Equals(other.Comparer);
	// Commonly, both comparers will be the default comparer (and reference-equal). Avoid a virtual method call to Equals() in that case.
	...
	public override void IntersectWith(IEnumerable<(T Key, int Value)> other)
	{
		ArgumentNullException.ThrowIfNull(other);
		if (Length == 0)
			return;
	...
	public override bool IsProperSupersetOf(IEnumerable<(T Key, int Value)> other)
	{
		...
		if (other is SumSet<T> asSorted && HasEqualComparer(asSorted))
		{
			if (asSorted.Length >= Length)
				return false;
			SumSet<T> pruned = GetViewBetween(asSorted.Min, asSorted.Max);
			foreach (var item in asSorted)
			{
				if (!pruned.Contains(item))
					return false;
	...
	private bool IsSubsetOfSortedSetWithSameComparer(SumSet<T> asSorted)
	{
		SumSet<T> prunedOther = asSorted.GetViewBetween(Min, Max);
		foreach (var item in this)
		{
			if (!prunedOther.Contains(item))
				return false;
	...
	private void RemoveAllElements(IEnumerable<(T Key, int Value)> collection)
	{
		T? min = Min;
		T? max = Max;
		foreach (var item in collection)
	...
	public override SumSet<T> RemoveAt(int index)
	{
		if (root == null)
			return this;
		FindForRemove(index, out SumSet<T>.Node? parent, out SumSet<T>.Node? grandParent, out SumSet<T>.Node? match, out SumSet<T>.Node? parentOfMatch);
		// Move successor to the matching node position and replace links.
		if (match != null)
		{
			ReplaceNode(match, parentOfMatch!, parent!, grandParent!);
	...
	public virtual bool RemoveValue(T item)
	{
		...
		while (current != null)
		{
			...
			// We don't need to compare after we find the match.
			int order = foundMatch ? -1 : Comparer2.Compare(item, current.Item.Key);
			if (order == 0)
			{
				// Save the matching node.
				foundMatch = true;
	...
	public override void SymmetricExceptWith(IEnumerable<(T Key, int Value)> other)
	{
		ArgumentNullException.ThrowIfNull(other);
		if (Length == 0)
		{
	...
	private void SymmetricExceptWithSameComparer(SumSet<T> other)
	{
		Debug.Assert(other != null);
		Debug.Assert(HasEqualComparer(other));
		foreach (var item in other)
	...
	private void SymmetricExceptWithSameComparer((T Key, int Value)[] other, int count)
	{
		Debug.Assert(other != null);
		Debug.Assert(count >= 0 && count <= other.Length);
		if (count == 0)
		...
		for (int i = 0; i < count; i++)
		{
			while (i < count && i != 0 && Comparer2.Compare(other[i].Key, previous.Key) == 0)
				i++;
			if (i >= count)
				break;
			var current = other[i];
	...
	public override void UnionWith(IEnumerable<(T Key, int Value)> other)
	{
		ArgumentNullException.ThrowIfNull(other);
		SumSet<T>? asSorted = other as SumSet<T>;
		TreeSubSet? treeSubset = this as TreeSubSet;
		if (treeSubset != null)
			VersionCheck();
		if (asSorted != null && treeSubset == null && Length == 0)
		...
		if (asSorted != null && treeSubset == null && HasEqualComparer(asSorted) && (asSorted.Length > Length / 2))
		{
			...
			while (!mineEnded && !theirsEnded)
			{
				int comp = Comparer2.Compare(mine.Current.Key, theirs.Current.Key);
				if (comp < 0)
				{
					merged[c++] = mine.Current;
					mineEnded = !mine.MoveNext();
	...
	internal sealed class TreeSubSet : SumSet<T>
	{
		...
		internal override T MaxInternal
		{
			get
			{
				...
				while (current != null)
				{
					int comp = _uBoundActive ? Comparer2.Compare(_max, current.Item.Key) : 1;
					if (comp < 0)
						current = current.Left;
					else
					{
		...
		internal override T MinInternal
		{
			get
			{
				...
				while (current != null)
				{
					int comp = _lBoundActive ? Comparer2.Compare(_min, current.Item.Key) : -1;
					if (comp > 0)
						current = current.Right;
					else
					{
		...
		internal override bool IsWithinRange(T item)
		{
			int comp = _lBoundActive ? Comparer2.Compare(_min, item) : -1;
			if (comp > 0)
				return false;
