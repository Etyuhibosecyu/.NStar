		...
		else if (sourceIndex >= destinationIndex)
		{
			...
			else
			{
				...
				for (int sourceCurrentIntIndex = sourceIntIndex + 1, destinationCurrentIntIndex = destinationIntIndex; destinationCurrentIntIndex < destinationEndIntIndex; sourceCurrentIntIndex++, destinationCurrentIntIndex++)
				{
					destinationBits[destinationCurrentIntIndex] = (uint)buff;
					buff >>= BitsPerInt;
					if (sourceCurrentIntIndex < sourceBits.Count) buff |= ((ulong)sourceBits[sourceCurrentIntIndex]) << bitsOffset;
				}
				ulong destinationMask = ((ulong)1 << destinationEndBitsIndex + 1) - 1;
				buff &= destinationMask;
				destinationBits[destinationEndIntIndex] &= (uint)~destinationMask;
...
[ComVisible(true)]
[Serializable]
public unsafe class NBitList : ListBase<bool, NBitList>, ICloneable
{
	private uint* _items;
	...
	public NBitList(int length, bool defaultValue)
	{
		if (length < 0)
			throw new ArgumentOutOfRangeException(nameof(length));
	...
	public NBitList(int length, uint* ptr)
	{
		if (length < 0)
			throw new ArgumentOutOfRangeException(nameof(length));
		_items = (uint*)Marshal.AllocHGlobal(sizeof(uint) * (_capacity = GetArrayLength(length, BitsPerInt)));
	...
	public NBitList(IEnumerable bits)
	{
		if (bits == null)
			throw new ArgumentNullException(nameof(bits));
		else if (bits is NBitList bitList)
		{
	...
	private protected override Func<int, NBitList> CapacityCreator => CapacityCreatorStatic;

	private static Func<int, NBitList> CapacityCreatorStatic => x => new(x);

	private protected override Func<IEnumerable<bool>, NBitList> CollectionCreator => CollectionCreatorStatic;

	private static Func<IEnumerable<bool>, NBitList> CollectionCreatorStatic => x => new(x);
...
internal override void SetInternal(int index, bool value)
	...
	public virtual NBitList And(NBitList value)
	{
		if (value == null)
			throw new ArgumentNullException(nameof(value));
		if (_size != value._size)
	...
	public virtual NBitList Or(NBitList value)
	{
		if (value == null)
			throw new ArgumentNullException(nameof(value));
		if (_size != value._size)
	...
	public virtual NBitList Xor(NBitList value)
	{
		if (value == null)
			throw new ArgumentNullException(nameof(value));
		if (_size != value._size)
	...
	public virtual object Clone()
	{
		NBitList bitList = new(_size, _items);
		return bitList;
	}

	private protected override void Copy(ListBase<bool, NBitList> source, int sourceIndex, ListBase<bool, NBitList> destination, int destinationIndex, int count)
	{
		NBitList source2 = source as NBitList ?? throw new ArgumentException(null, nameof(source));
		NBitList destination2 = destination as NBitList ?? throw new ArgumentException(null, nameof(destination));
	...
	private protected override bool EqualsInternal(IEnumerable<bool>? collection, int index, bool toEnd = false)
	{
		...
		if (collection == null)
			throw new ArgumentNullException(nameof(collection));
		if (collection is NBitList bitList)
		{
			if (index > _size - bitList.Length)
				throw new ArgumentOutOfRangeException(nameof(index));
			if (toEnd && index < _size - bitList.Length)
	...
	public virtual void InsertRange(int index, IEnumerable collection)
	{
		...
		else if (collection is uint[] uintArray)
		{
			...
			InsertRange(index, new NBitList(collection));
	}
	...
	private protected override NBitList ReverseInternal(int index, int count)
	{
		for (int i = 0; i < count / 2; i++)
		{
			(this[index + i], this[index + count - i - 1]) = (this[index + count - i - 1], this[index + i]);
	...
	public virtual void SetRange(int index, IEnumerable collection)
	{
		...
		if (collection is NBitList bitList)
		{
			...
			SetRange(index, new NBitList(collection));
	}
