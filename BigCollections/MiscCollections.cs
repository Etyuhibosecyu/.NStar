...
[ComVisible(true)]
[Serializable]
public class Queue<T> : IEnumerable<T>, ICollection, ICloneable
{
	private T[] _array;
	private int _head;
	private int _tail;
...
public class BigQueue<T> : IEnumerable<T>, ICloneable
{
	...
	public virtual void Enqueue(T obj)
	{
		...
		{
			high = new(4);
			high.Enqueue(new(CapacityStep) { low = low, _size = low.Count });
		...
		else if (high != null)
		{
			...
			if (index == CapacityStep)
			{
				...
				high = new(4);
				high.Enqueue(new(_size) { high = temp, _size = temp.Count });
				high.Enqueue(new(_size));
				high.GetElement(high.Count - 1).Enqueue(obj);
	...
	public virtual void TrimExcess()
	{
		...
		{
			high.TrimExcess();
			high.GetElement(high.Count - 1).TrimExcess();
		}
	}
