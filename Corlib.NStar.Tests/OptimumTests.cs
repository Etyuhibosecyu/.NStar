...
public class OptimumTests
{
	...
	public void TestDirect()
	{
		...
		static void ProcessA(G.IEnumerable<MpzT> a, ImmutableArray<BigInteger> list3)
		{
			...
			Assert.IsTrue(@long == longOptimum);
			@double = a.Mean();
			doubleOptimum = list3.Length == 0 ? 0 : E.Aggregate(list3, 0d, (x, y) => x + (double)y) / list3.Length;
			Assert.IsTrue(@double == doubleOptimum);
			@double = a.Mean(x => x.PowerMod(new MpzT(power), new(mod)));
			doubleOptimum = list3.Length == 0 ? 0 : E.Aggregate(E.Select(list3, x => BigInteger.ModPow(x, power, mod)), 0d, (x, y) => x + (double)y) / list3.Length;
			Assert.IsTrue(@double == doubleOptimum);
			@double = a.Mean((x, index) => x.PowerMod(new MpzT(power * index), new(mod)));
			doubleOptimum = list3.Length == 0 ? 0 : E.Aggregate(E.Select(list3, (x, index) => BigInteger.ModPow(x, power * index, mod)), 0d, (x, y) => x + (double)y) / list3.Length;
			Assert.IsTrue(@double == doubleOptimum);
			...
			doubleOptimum = list3.Length == 0 ? 0 : E.Aggregate(E.Select(list3, x => (long)(ulong)(x / 4294967296 % 4294967296 * 4294967296 + x % 4294967296)), 0d, (x, y) => x + y) / list3.Length;
			Assert.IsTrue(@double == doubleOptimum);
			@double = a.Mean((x, index) => (long)(x * (index % 5)));
			doubleOptimum = list3.Length == 0 ? 0 : E.Aggregate(E.Select(list3, (x, index) => (long)(ulong)(x * (index % 5) / 4294967296 % 4294967296 * 4294967296 + x * (index % 5) % 4294967296)), 0d, (x, y) => x + y) / list3.Length;
			Assert.IsTrue(@double == doubleOptimum);
			mpzT = a.Min();
